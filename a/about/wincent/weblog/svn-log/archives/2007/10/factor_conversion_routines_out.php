...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>
      Git activity log: Factor conversion routines out into per-character
      functions (wikitext, c5ca963)
    </title>
    ...
    <link
      rel="prev"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/flesh_out_accessor_methods_for.php"
      title="Flesh out accessor methods for the internal link prefix (wikitext, 17856f8)"
    />

    <link
      rel="next"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/add_url_encoding_for_internal.php"
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/flesh_out_accessor_methods_for.php"
            >&laquo; Flesh out accessor methods for the internal link prefix
            (wikitext, 17856f8)</a
          >
          |
        ...
        <h3>
          Factor conversion routines out into per-character functions (wikitext,
          c5ca963)
        </h3>

        <p>
          Extract two new functions, one for converting a single UCS-2 character
          into a (possibly multi-byte) UTF-8 representation, and another for
          performing the reverse operation.
        </p>

        <p>
          This change will be useful when sanitizing internal link targets in
          the future; such link targets must be URL-encoded, which means
          converting them to UTF-8, performing the encoding, and then converting
          back into UCS-2 for merging into the output stream (whose working
          format is UCS-2). The functions can be used to efficiently convert on
          a per-character basis and thus avoid potentially expensive allocations
          of temporary strings; in fact the whole thing can be done in pure C
