...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>
      Git activity log: Call Data_Wrap_Struct on ary structs (wikitext, 7f506d3)
    </title>
    ...
    <link
      rel="prev"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/02/extract_ary_free_out_into_sepa.php"
      title="Extract ary_free() out into separate file (wikitext, 840b62d)"
    />

    <link
      rel="next"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/02/add_gc_wrap_ary_macro_wikitext.php"
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/02/extract_ary_free_out_into_sepa.php"
            >&laquo; Extract ary_free() out into separate file (wikitext,
            840b62d)</a
          >
          |
          ...
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/02/add_gc_wrap_ary_macro_wikitext.php"
            >Add GC_WRAP_ARY macro (wikitext, f4b4579) &raquo;</a
          >
        </p>
        ...
        <p>
          There is a minuscule overhead introduced by this commit (the
          allocation of three Ruby objects per parse, which in turn means three
          more objects involved in Ruby's mark-and-sweep Garbage Collection
          scheme) which brings with it a robustness enhancement: if we exit the
          parse method irregularly (by an exception) the three ary structs will
          be properly freed rather than leaking.
        </p>
        ...
        <p>
          ...
          really ever happen in practice (the only times when we would exit via
          an exception are when we run out of memory, in which case we're
          totally hosed anyway and process termination is just around the
          corner, and when we have invalidly encoded UTF-8 input; this latter
          scenario may be an issue if an attacker specifically tries to consume
          resources by repeatedly feeding in hand-crafted bad input, but I don't
          know whether such an attack would be practically feasible).
        </p>
        ...
        <p>
          ...
          refrain from collecting it prematurely. Without the volatile keyword
          the compiler will see that the pointer is initialized and then never
          used again and is free to optimize it away to nothing, which then
          allows Ruby to go ahead and free the structure before its time. The
          volatile keyword here prevents this optimization, at least in GCC, and
          is apparently an &quot;accepted hack&quot; that is used in the Ruby
          source itself.
        </p>

        <p>Signed-off-by: Greg Hurrell &lt;greg@hurrell.net&gt;</p>

        <div id="a004173more"><div id="more"></div></div>
