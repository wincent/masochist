...
describe('getParseTable()', () => {
  ...
  it('returns a ParseTable for the full grammar', () => {
    ...
    expect('\n' + stringifyGrammar(getAugmentedGrammar(grammar)))
      ...
      r24: DirectivesOpt → DirectiveList { $$ = $1; }
      r25: DirectivesOpt → ε { $$ = null; }
      r26: DirectiveList → Directive { $$ = [$1]; }
      r27: DirectiveList → DirectiveList Directive { $1.push($2); $$ = $1; }
      r28: Directive → AT NAME {
        $$ = {
          kind: 'DIRECTIVE',
          name: $2.contents,
        };
      }
      r29: SelectionSet → OPENING_BRACE SelectionList CLOSING_BRACE { $$ = $2; }
      r30: SelectionSetOpt → SelectionSet { $$ = $1; }
      r31: SelectionSetOpt → ε { $$ = null; }
      r32: SelectionList → Selection { $$ = [$1]; }
      r33: SelectionList → SelectionList Selection { $1.push($2); $$ = $1; }
      r34: Selection → Field { $$ = $1; }
      r35: Field → NAME SelectionSetOpt {
        $$ = {
          kind: 'FIELD',
          alias: null,
          directives: null,
      ...
      r36: Field → Alias NAME SelectionSetOpt {
        $$ = {
          kind: 'FIELD',
          alias: $1,
          directives: null,
      ...
      r37: Alias → NAME COLON { $$ = $1.contents; }
      "
    `);
  });
});
