  ...
  describe('a()/an()', () => {
    ...
    it('wraps a matcher so it can be augmented without mutation', () => {
      ...
      const lexer = new Lexer((api) => {
        ...
        const SEPARATOR = match(':').onMatch(() => calls.push('SEPARATOR'));
      ...
      expect(calls).toEqual(['ORIGINAL', 'COPY', 'SEPARATOR', 'ORIGINAL']);
    });
  });
  ...
  describe('allOf()', () => {
    ...
    beforeEach(() => {
      lexer = new Lexer((api) => {
        ...
        return () => {
          const text = consume(allOf(match('foo'), match('bar'), match('baz')));
  ...
  describe('maybe()', () => {
    it('produces an optional match', () => {
      const lexer = new Lexer((api) => {
        ...
        return () => {
          const text = consume(
            sequence(match('foo'), maybe(match('bar')), match('baz')),
          );
  ...
  describe('oneOf()', () => {
    it('matches using the first matching matcher in a collection', () => {
      const lexer = new Lexer((api) => {
        ...
        return () => {
          const text = consume(oneOf(match('foo'), match('bar'), match('baz')));
