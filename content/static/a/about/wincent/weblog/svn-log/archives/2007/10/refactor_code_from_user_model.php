...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>
      Git activity log: Refactor code from User model into Authentication module
      (wincent.dev, f625239)
    </title>
    ...
    <link
      rel="prev"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/specs_for_session_controller_a.php"
      title="Specs for session controller and tweaks to get them passing (wincent.dev, 067ca6f)"
    />

    <link
      rel="next"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/add_application_controller_spe.php"
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/specs_for_session_controller_a.php"
            >&laquo; Specs for session controller and tweaks to get them passing
            (wincent.dev, 067ca6f)</a
          >
          |
          ...
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/10/add_application_controller_spe.php"
            >Add application controller specs (wincent.dev, f7ec927) &raquo;</a
          >
        </p>
        ...
        <h3>
          Refactor code from User model into Authentication module (wincent.dev,
          f625239)
        </h3>

        <p>
          At the moment the authentication logic is spread across several sites:
          the User model, the ApplicationController (and by inheritance all its
          subclasses), and the Authentication module itself.
        </p>

        <p>
          This commit tries to centralize as much of this code as possible
          inside the Authentication module with the goal of keeping the
          authentication code in one place. It is unavoidable that an
          authentication mechanism be involved with both controllers and models
          (and later views), but at least we can store the logic in a single
          file, clearly grouped using submodules (Authentication::Model,
          Authentication::Controller and so forth), and then incorporate it
          elsewhere using extend and include.
        </p>

        <p>
          There still is quite a bit of explicit authentication logic in the
          user model itself (mostly validation and callback related), and I am
          not sure what to do about that. Although the idea of keeping
          authentication code bundled together in one place is superficially
          appealing, if taken to far it can actually make the code less
          readable. As such, the refactoring done in this commit is relatively
