...
3.  Add [a hook](https://github.com/wincent/wincent/blob/main/support/hooks/pre-push) to `.git/hooks/pre-push` that will update the local `master` whenever `main` is pushed, and a [remote `post-receive` hook](https://github.com/wincent/wincent/blob/main/support/hooks/post-receive) that does the analogous job on git.typechecked.net, where I am running my own `git-daemon`.

4.  Push `main` for the first time.

5.  Set the default branch to `main` in GitHub, GitLab, and BitBucket.

6.  Additionally, on git.typechecked.net, it is necessary to update the `HEAD` so that `git clone` operations from it check out `main` instead of `master`:
