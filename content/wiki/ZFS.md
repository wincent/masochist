---
tags: os.x wiki
cache_breaker: 1
---

[ZFS](/wiki/ZFS) or the [Zettabyte File System](/wiki/Zettabyte_File_System) is a revolutionary file-system made by [Sun Microsystems](/wiki/Sun_Microsystems) and ported by [Apple](/wiki/Apple) to [Mac OS X](/wiki/Mac_OS_X) for [Leopard](/wiki/Leopard). At the time of writing, it has never enjoyed "official" status as a supported read-write file system for Mac OS X, and the official [project page](http://zfs.macosforge.org/) indicated that the project was discontinued in October 2009. [Speculation](http://www.reddit.com/r/programming/comments/9x4sb/zfs_project_on_macosforgecom_officially/) indicates that this might have occurred due to licensing difficulties.

# [ZFS](/wiki/ZFS) links

-   [ZFS](/wiki/ZFS) project page (Sun): <http://www.opensolaris.org/os/community/zfs/>
    -   [ZFS](/wiki/ZFS) news article at [Sun](/wiki/Sun): <http://www.sun.com/2004-0914/feature/>
-   ZFS project page at Mac OS Forge: <http://zfs.macosforge.org/>
-   [Wikipedia](/wiki/Wikipedia) article on [ZFS](/wiki/ZFS): <http://en.wikipedia.org/wiki/ZFS>
-   Article on [RAID-Z](/wiki/RAID-Z): <http://blogs.sun.com/bonwick/entry/raid_z>
    -   Summary of the article: <http://solaris.reys.net/english/archives/93-VozmoZhnosti_ZFS_RAID-Z.html>
-   Performance comparison with hardware [RAID](/wiki/RAID):
    -   <http://milek.blogspot.com/2006/08/hw-raid-vs-zfs-software-raid.html>
    -   <http://milek.blogspot.com/2006/08/hw-raid-vs-zfs-software-raid-part-ii.html>
