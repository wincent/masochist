...
I've been using [RapidSSL](/wiki/RapidSSL) for my [SSL](/wiki/SSL) certificates for a few years now, using [ServerTastic](http://www.servertastic.com/) because it's been the cheapest reseller (\$50 for a 5/year renewal). Right now I have one cert for wincent.dev and another for wincent.dev (buying two separate certs is still considerably cheaper than getting a wildcard cert, although one day I'll probably cave in and get a wildcard cert in order to gracefully handle and redirect HTTPS requests coming in to wincent.dev and possibly other subdomains as well).

This year I was forced into renewing a little early because Chrome started complaining about the signature algorithm used on the older of my two certs (the certificate in question was for wincent.dev, and set to expire on 23 May 2013; just under 11 months from now). I had originally thought that by the time it expired I might have migrated all my SSL traffic onto wincent.dev, but that hasn't happened. This is the first time I've been "bitten" by my selection of cheaper certificates.
...
At this point we can restart sendmail and test:
    ...
    openssl s_client -starttls smtp -connect wincent.dev:25 -CApath /etc/pki/tls/certs
...
And verify in [Mail.app](/wiki/Mail.app) and on the [command line](/wiki/command_line):
    ...
    # test secure IMAP
    openssl s_client -connect wincent.dev:993  -CApath /etc/pki/tls/certs

    # test secure POP
    openssl s_client -connect wincent.dev:995  -CApath /etc/pki/tls/certs
