...
# What's changed since [the 1.2.1 release](/blog/command-t-1.2.1-released)

This release adds the `g:CommandTMaxCachedDirectories` option, which means you can use multiple, per-directory caches (useful if you tend to switch around a lot among a set of directories). It also adds a new "jumplist" finder (thanks to Marian Schubert for the patch), accessible using `:CommandTJump` (which you can bind to something useful like `<Leader>j`). The neat thing about the jumplist is that, Vim settings permitting, it persists across restarts of Vim, making the jumplist finder a great option for moving around frequently edited files, regardless of where they live on disk.

As always, a full change-log appears under HISTORY in the documentation, and you can explore the commits in the release [here](/repos/command-t/tags/1.3). (Note: the integrated repository browser that I'm linking to here is still relatively new and doesn't have a full feature set yet.)
