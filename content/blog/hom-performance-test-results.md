...
Doing a "select" HOM on an array of one element is not exactly an optimal usage case, regardless of the technique used, so performance is very similar across the board. The [new "chained" version](http://wincent.dev/a/about/wincent/weblog/archives/2006/11/hom_improvement.php) of the "select" HOM beats the old version by a significant amount (about twice as fast) and can also do things the original version couldn't do (nesting to 3 or more levels).
