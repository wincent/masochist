...
I just made a curious realization while trying to build [MySQL](https://wincent.dev/wiki/MySQL) for [Leopard](https://wincent.dev/wiki/Leopard): I don't know how to create new daemon user. No doubt this is ridiculously easy with Mac OS X Server, but in the non-server version it's far from obvious.

The NetInfo Manager utility has finally be laid to rest, but so have [command line](https://wincent.dev/wiki/command%20line) tools like `adduser`. The `/etc/passwd` file starts with this info:
