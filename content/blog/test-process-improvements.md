...
I had some failing tests in place (`WO_TEST_FAIL`) to serve as reminders to go ahead and implement real tests at a later date. This is mostly in cases where the tests are fiendishly difficult to write (testing [GUI](http://www.wincent.com/wiki/GUI) elements, for example) and I wasn't sure how to proceed.
...
My next step will then be to set up truly continuous automated testing for better [continuous integration](http://www.wincent.com/wiki/continuous%20integration). The idea is to perform the following in the background in an endless loop:
...
2.  The amazing [RSpec](http://www.wincent.com/wiki/RSpec) [BDD](http://www.wincent.com/wiki/BDD) framework which leverages the everything-is-an-object design of [Ruby](http://www.wincent.com/wiki/Ruby) to improve the readability of your specs:
...
When I saw [RSpec](http://www.wincent.com/wiki/RSpec) I wanted to modify WOTest to use a similar syntax (`should` and `shouldNot` messages added via a category on `NSObject`) using [Higher Order Messaging](http://www.wincent.com/wiki/Higher%20Order%20Messaging) to allow expressions like:
...
I guess it could be done, but you could never go the full distance in Objective-C. That's one of the reasons I'm looking forward to [Leopard](http://www.wincent.com/wiki/Leopard); I'm hoping that the RubyCocoa bridge will allow me to write specs in [Ruby](http://www.wincent.com/wiki/Ruby) that can be used to test [Objective-C](http://www.wincent.com/wiki/Objective-C) objects. Maybe using [RSpec](http://www.wincent.com/wiki/RSpec) itself, or providing a bridge layer to tap into the existing WOTest machinery.
