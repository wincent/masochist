...
You can browse the source code repo [here](http://git.wincent.com/wikitext.git) or [here](/repos/wikitext), or download the [gem](/wiki/gem) from the [rubygems.org downloads page](http://rubygems.org/gems/wikitext). For an overview of how to use the extension check out the docs [here](/products/wikitext/doc/). If you have [RubyGems](/wiki/RubyGems) then you can install it with either `gem install --pre wikitext` or `sudo gem install --pre wikitext`. (To get the previous stable release, `gem install wikitext` or `sudo gem install wikitext` will suffice.)
