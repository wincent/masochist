...
> I've recently switched from using [svn](https://wincent.dev/wiki/svn) to [git](https://wincent.dev/wiki/git), and I'm about to switch from using [WordPress](https://wincent.dev/wiki/WordPress) to [MovableType](https://wincent.dev/wiki/MovableType) too.
>
...
The reason why I never used to upgrade my Movable Type via Subversion is that [Six Apart](https://wincent.dev/wiki/Six%20Apart) (the makers of Movable Type) don't provide public access to their [version control system](https://wincent.dev/wiki/version%20control%20system); they instead distribute new releases of Movable Type as tarballs or zip archives.

As such, it wasn't possible to update an existing, customized install using Subversion (see "[Upgrading WordPress using Subversion](https://wincent.dev/wiki/Upgrading%20WordPress%20using%20Subversion)"). I did make some notes on making the upgrade as smooth as possible with the aid of things like [Interarchy](https://wincent.dev/wiki/Interarchy)'s "mirror upload" feature (see "[Movable Type upgrade notes](https://wincent.dev/wiki/Movable%20Type%20upgrade%20notes)"), but it was never going to be as convenient as a Subversion-based upgrade.
...
I'm not sure whether this is better than the download/extract/upload cycle, but merging with Git should be much more straightforward than merging with your [SFTP](https://wincent.dev/wiki/SFTP) client... Of course, it would still be a lot easier if Six Apart offered Subversion, Git or even [CVS](https://wincent.dev/wiki/CVS) access to a repository containing the public releases.
