...
Along with my other development work, since the start of the year I've been working on an object-oriented templating system called Walrus. Its functionality is similar to that of the [Cheetah](https://wincent.dev/wiki/Cheetah) templating engine but it's written in [Ruby](https://wincent.dev/wiki/Ruby) instead of in Python. It's a wheel that I was forced to reinvent for two reasons: one, the fact that I don't know Python; and two, problems with Cheetah (missing features or undesired behaviours) which I couldn't correct because I don't know Python... At this stage it offers only a subset of the Cheetah functionality but it's powerful enough to do what I designed it for: making documentation for display in Apple's Help Viewer.

It turns out that what started out as a simple project ended up becoming quite a bit more complex. But I'm very glad I did it because I learned a great deal along the way and my familiarity with Ruby has gone through the roof: something that will be very useful as I work on the [Rails](https://wincent.dev/wiki/Rails) rewrite of this site and start using the Ruby scripting bridge that [Apple](https://wincent.dev/wiki/Apple) will ship with [Leopard](https://wincent.dev/wiki/Leopard).

Walrus is essentially a compiler that reads files written in a special templating markup (almost identical to that used by Cheetah) and compiles them down to Ruby code. The final output is produced by "filling" (running) the compiled templates. There is a `walrus` [command line](https://wincent.dev/wiki/command%20line) tool that serves as a front end and makes it easy to process templates in batch style. At its core Walrus employs a [packrat](https://wincent.dev/wiki/packrat) parser which can recognize any [PEG](https://wincent.dev/wiki/PEG) and even accepts constructs that wouldn't usually be legal in a [PEG](https://wincent.dev/wiki/PEG) such as left-recursive productions. The parser is capable of producing arbitrarily complex [ASTs](https://wincent.dev/wiki/ASTs). Although I'm using it to produce help documentation the design is general enough to be used in any situation where you want to transform text from one format to another. All this means that it is a tool that could end up proving quite useful.

## The output

It's been very gratifying over the last few days to see it finally come to the point where I can start leveraging it for use in my own applications. [Click here for a screenshot](https://wincent.dev/a/about/wincent/weblog/hextrapolate_help_screenshot.png) of the help pages for Hextrapolate.

I want the documentation to look beautiful to the user, so the style sheets are based on the current Apple look and feel in [Tiger](https://wincent.dev/wiki/Tiger). But I want the [HTML](https://wincent.dev/wiki/HTML) source to look beautiful as well; this is achieved principally through careful design of the [HTML](https://wincent.dev/wiki/HTML)/[CSS](https://wincent.dev/wiki/CSS) to minimize the amount of clutter in the markup, and then by cleaning up the whitespace using [Tidy](https://wincent.dev/wiki/Tidy):
...
So how is this different from the traditional [PHP](https://wincent.dev/wiki/PHP)-style model of template re-use? In that model you define a master container and then you split off common elements into separate documents (header, footer etc). Each page basically looks like this:
