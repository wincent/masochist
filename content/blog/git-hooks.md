...
Earlier this month I [wrote](https://wincent.dev/a/about/wincent/weblog/archives/2007/07/musings_on_subv.php):

> It seems inevitable that I'll move, sooner or later, to [Git](https://wincent.dev/wiki/Git). There are still a couple of gaps that would need to be filled in, but 90% of what I need is there.

Yesterday I talked about [getting up a web interface](https://wincent.dev/a/about/wincent/weblog/archives/2007/07/git_notes.php) for my public repositories; I've migrated three so far. The `gitweb` interface largely supercedes the existing "[Subversion log](https://wincent.dev/a/about/wincent/weblog/svn-log/archives/)" interface, at least for my [open source](https://wincent.dev/wiki/open%20source) projects.
...
The first automatic Git-driven update to the log can be seen [here](https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/07/initial_import_snippets_4cfa16.php).

This is done with a Git `post-receive` hook script that updates the log (actually just a [Movable Type](https://wincent.dev/wiki/Movable%20Type) [weblog](https://wincent.dev/wiki/weblog)) via [XML-RPC](https://wincent.dev/wiki/XML-RPC). Doing this is a little bit trickier than it is with [Subversion](https://wincent.dev/wiki/Subversion).
