...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>wincent.com: activation: denial causes</title>
    ...
    <div style="text-align: center; font-family: 'Lucida Grande'; margin: 5px 250px 5px 250px; padding: 5px; font-size: x-small; border: 2px solid silver; -webkit-border-radius: 0.5em; -moz-border-radius: 0.5em; background: #fefefe;">
      ...
      <tr>
        ...
        <td style="background-image: url(/a/gfx/header/center-background.jpg); background-repeat: repeat;" colspan="11" align="center" valign="middle">          <!-- Begin: header content -->
          ...
          <img src="/a/gfx/support-header.jpg" width="207" height="23" alt="product and site news" align="middle"/>
          <!-- End: header content -->
        </td>
  ...
  <div id="breadcrumbs">
    <p><a href="http://www.wincent.com/">Main</a>&nbsp;&gt;&nbsp;<a href="../../">Support</a>&nbsp;&gt;&nbsp;<a href="../">Activation</a>&nbsp;&gt;&nbsp;Denial Causes    </p>
  </div>
    <div id="container">
      <div id="left">
        <div class="sidebar">
      ...
      <div id="center">
        <div class="content">
          <h3>Product activation: causes of denied activation requests</h3>
          
          <h4>Introduction</h4>
          
          <p>In the general case product activation is a simple two-step process:</p>
          
          <ol>
            <li>Product sends activation request to server</li>
            <li>Server replies with activation certificate</li>
          </ol>
          
          <p>There are a number of reasons for which the server may deny an activation request. In these cases the server replies with a reason for the denial and a link back to a page on the server (this page) where you can read more information about why the request was denied. In normal use end users should never have their activation request denied although <em>if</em> they activate the software too many times then they <em>will</em> eventually exceed the activation limit (nevertheless, the limits are quite liberal so most users should never exceed them).</p>
          
          <p>In all other cases activation will only be denied due to a malformed or illegal activation request. An attacker would receive one of these responses if he or she were to try and circumvent or manipulate the product activation system using a network tool. Apart from being illegal in many states and countries this kind of attack is not likely to prove very fruitful: a &quot;successful&quot; attack would only permit the attacker to use a valid license to run a copy of the software on the machine for which the activation certificate has been generated; in other words, activate the software using a valid license. Without a valid license an activation certificate has no validity.  Rather than staging elaborate attacks it's generally faster and easier to purchase a license and receive a valid code, enter the license code in the product interface, and activate the license by clicking the &quot;Verify&quot; button.</p>
          
          <h4>Summary of denial reasons</h4>
          
          <ul>
            <li><a href="#improperly-formed">&quot;Improperly formed activation request&quot;</a></li>
            <li><a href="#no-valid-license">&quot;Activation request does not match any valid license&quot;</a></li>
            <li><a href="#license-blacklisted">&quot;License blacklisted&quot;</a></li>
            <li><a href="#machine-limit">&quot;Activation limit (number of machines) exceeded&quot;</a></li>
            <li><a href="#total-limit">&quot;Activation limit (total activations) exceeded&quot;</a></li>
            <li><a href="#email-blacklisted">&quot;Email address associated with this license is blacklisted&quot;</a></li>
            <li><a href="#unknown-product">&quot;Unknown product code&quot;</a></li>
            <li><a href="#invalid-digest">&quot;Invalid cryptographic digest(s)&quot;</a></li>
          </ul>
          
          <h4>Detailed descriptions of denial reasons</h4>
          
          <h5><a id="improperly-formed" name="improperly-formed"></a>&quot;Improperly formed activation request&quot;</h5>
          
          <p>Any Wincent product that requires product activation must send a specially formatted activation request to the wincent.com server (or more precisely the secure.wincent.com subdomain) at the time of activation. The format is extremely simple and contains only two pieces of information (randomly generated numbers that could not be used to identify you personally if anybody were observing the activation). In normal use you should never see this denial reason because all Wincent products know how to produce a correctly formatted activation request.</p>
          
          <p>For more information about the privacy and security of the activation process see the <a href="../privacy/">Activation Privacy FAQ</a>.</p>
          
          <h5><a id="no-valid-license" name="no-valid-license"></a>&quot;Activation request does not match any valid license&quot;</h5>
          
          <p>The server refuses to issue activation certificates for invalid licenses. In normal use you should never see this denial reason because all Wincent products check the validity of their license codes before submitting an activation request.</p>
          
          <h5><a id="license-blacklisted" name="license-blacklisted"></a>&quot;License blacklisted&quot;</h5>
          
          <p>The server will not issue activation certificates for licenses that have been blacklisted. A licenses will be added to the blacklist if it is known to be illegally shared (for example, posted to the Internet or distributed in a pirate serial collection).</p>
          
          <h5><a id="machine-limit" name="machine-limit"></a>&quot;Activation limit (number of machines) exceeded&quot;</h5>
          
          <p>If you attempt to activate the product on too many machines then the server will deny activation. The server takes into consideration whether the license is a single or multi-user license, how many licenses have been issued, as well as allowing a reasonable number of multiple activations for people who upgrade their machines, transfer the software to a different computer, or wipe their hard drives. For example, if you have two five-user licenses you will be allowed to perform activation on ten times as many machines as a person with one single-user license. The activation limits are quite liberal and so it is unlikely that you will ever see this denial reason. The limits are liberal so that paying customers won't be inconvenienced by the activation process. Note that although the system will allow you to perform multiple activations with the same license this does not mean that you are authorized to install the software on more machines than the number for which you have a license. For example if you want to install the software on a second machine and you only have one single-user license then you must purchase another single-user license.</p>
          
          <p>The activation system is designed to control the damage that occurs when a license code gets leaked and mass-distributed on the Internet. If thousands of people attempt to use the same license code to activate the software then the system will quickly hit the activation limit. The vast majority of users are honest customers and they don't share their license codes with third parties and so are unlikely to hit their activation limit.</p>
          
          <p>If you exceed your activation limit and would like to have it reset please <a href="http://www.wincent.com/a/contact/mail/">contact Wincent</a>.</p>
          
          <h5><a id="total-limit" name="total-limit"></a>&quot;Activation limit (total activations) exceeded&quot;</h5>
          
          <p>In addition to the limit on the number of machines on which activation is permitted (see the previous section) there is a separate limit on the total number of activations. In other words, activation will be denied if the number of machines 'X' is exceeded, <em>or</em> if the total number of activations 'Y' is exceeded. At the time of writing the two limits 'X' and 'Y' are the same so you can either activate the software 'X' times on the same machine, or 1 time on 'X' different machines, or any other combination which does not exceed 'X'.</p>
          
          <p>As noted above, the activation limits are quite liberal so as to ensure that paying customers are not inconvenienced by the activation process. Although the limits may be liberal this does not mean that you are authorized to install the software on multiple machines unless you have a suitable multi-user license to cover that number of machines. For example if you want to install the software on a second machine and you only have one single-user license then you must purchase another single-user license.</p>
          
          <p>If you exceed your activation limit and would like to have it reset please <a href="http://www.wincent.com/a/contact/mail/">contact Wincent</a>.</p>
          
          <h5><a id="email-blacklisted" name="email-blacklisted"></a>&quot;Email address associated with this license is blacklisted&quot;</h5>
          
          <p>Like the &quot;License blacklisted&quot; denial describe above, the server will not issue activation certificates for any license that is associated with a blacklisted email address. An email address will be blacklisted if it is known to be illegally shared in conjunction with a license code (for example, posted to the Internet or distributed in a pirate serial collection). Obviously, telling someone your email address is fine but telling them your email address <em>and</em> your license code is not allowed (with the obvious exception that if you have a five-user license then each of those five users must know the email address and license code in order to activate the software).</p>
          
          <h5><a id="unknown-product" name="unknown-product"></a>&quot;Unknown product code&quot;</h5>
          
          <p>Each Wincent product has a unique numeric product code that identifies it; for example, the product code for Synergy Advance is 1. If an activation request is received that corresponds to an unknown product code then the server will automatically deny the request. In normal use you should never see this denial reason because all Wincent products will only ever send a known product code to the activation server. Even an attacker is unlikely to see this denial reason because any activation request containing an unknown product code will be denied at a much earlier point with &quot;Activation request does not match any valid license&quot; message.</p>
          
          <h5><a id="invalid-digest" name="invalid-digest"></a>&quot;Invalid cryptographic digest(s)&quot;</h5>
          
          <p>Cryptographic digest functions are used to protect your privacy when communicating with the activation server (see the <a href="../privacy/">Privacy FAQ</a> for more information). This means that instead of transmitting information about you, your machine or your license to the activation server a cryptographic digest is used (also known as a &quot;one-way hash&quot;) to transmit two pieces of information which to any outside observer would appear to be random numbers. The digest is called &quot;one-way&quot; because it is impossible to work backwards from the digest to find out what information was used to create it.</p>
          
          <p>In normal use you should never see this denial reason because all Wincent products produce validly formatted cryptographic digests. In fact, even an attacker is unlikely to see this denial reason because if he or she attempts to submit an activation request containing invalid digests to the server then the request will be denied at a much earlier point with an &quot;Activation request does not match any valid license&quot; message.</p>
