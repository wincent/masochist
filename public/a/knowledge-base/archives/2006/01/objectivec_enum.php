...
<link rel="prev" href="http://www.wincent.com/a/knowledge-base/archives/2006/01/security_notes.php" title="Security notes, Red Hat Enterprise Linux and rssh" />

<link rel="next" href="http://www.wincent.com/a/knowledge-base/archives/2006/01/locking_doublec.php" title="Locking, double-checked locking and speed" />
...
<body>
  ...
  <div id="breadcrumbs">
    <p><a href="http://www.wincent.com/">Main</a>&nbsp;&gt;&nbsp;<a href="../../../">Knowledge Base</a>&nbsp;&gt;&nbsp;<a href="../../">Archives</a>&nbsp;&gt;&nbsp;<a href="../">2006</a>&nbsp;&gt;&nbsp;01    </p>
  </div>
...
<p align="right">
<a href="http://www.wincent.com/a/knowledge-base/archives/2006/01/security_notes.php">&laquo; Security notes, Red Hat Enterprise Linux and rssh</a> |

<a href="http://www.wincent.com/a/knowledge-base/">Main</a>
| <a href="http://www.wincent.com/a/knowledge-base/archives/2006/01/locking_doublec.php">Locking, double-checked locking and speed &raquo;</a>
...
<p>I posted a <a href="http://www.wincent.com/a/about/wincent/weblog/archives/2006/01/locking_doublec.php">short article the other day on double-checked locking</a> and in the process of trying to get to the bottom of the memory barrier question I found myself reading <a href="http://rentzsch.com/papers/atomicity">this article</a> by <a href="http://rentzsch.com/">Jonathan Rentzsch</a> (<a href="http://www-128.ibm.com/developerworks/library/pa-atom/">also published on the IBM site in modified form</a>). One thing leads to another and I found myself reading his <a href="http://rentzsch.com/papers/loveHateObjC">love/hate paper on Objective-C</a>, then this one on <a href="http://rentzsch.com/papers/improvingCocoaObjCEnumeration">Objective-C enumeration</a> and finally that took me to Michael Tsai's <a href="http://mjtsai.com/blog/2003/12/08/cocoa_enumeration.html">article on the same subject</a>.</p>

<p>Well, I'll throw my hat in the ring too...</p>

<div id="a000187more"><div id="more">
<p>First up, I share Jonathan's and Michael's discontent with the standard enumeration idiom. It's always bugged me that I had to do stuff like this:</p>

<pre>NSEnumerator *outerEnumerator = [array objectEnumerator];
...
<p>I don't like having to worry about name clashes (<tt>outerEnumerator</tt> vs <tt>innerEnumerator</tt>). I don't like having to use three lines of code to set up the enumeration. I don't really care about the use of an assignment within a conditional expression (in the <tt>while</tt> statements) because I can tell GCC to warn me about places where it thinks I might have done it by accident, and I can explicitly suppress those warnings in places where I am sure that's what I want to do by using double parentheses as in the example above.</p>

<p>So Jonathan has his <a href="http://rentzsch.com/share/nsenumerate.h">own preprocessor macros to address his concerns</a>. Michael has a <a href="http://mjtsai.com/blog/2003/12/08/cocoa_enumeration.html">different macro</a>. Here's my own take on the idea:</p>
...
<pre>WOEnumerate(fileArray, fileName)
    NSLog(@"Filename: %@", filename);</pre>

<p>Things to note about the solution:</p>

<ul>
<li>Like Michael, I do throw away compile-time type checking (the object is always of type <tt>id</tt>) for the sake of simplicity.</li>
<li>Like Jonathan, I make all of the variables local to the loop so that I don't have to worry about clashes.</li>
<li>I provide two macros, one for object enumeration (for example, for use with <tt>NSArray</tt> collections) and one for key enumeration (for example, for use with <tt>NSDictionary</tt> collections). It would be trivial to add a third macro, <tt>WOReverseEnumerate</tt>, that would use the <tt>reverseObjectEnumerator</tt> selector.</li>
<li>Like Michael, I cache both the method selector and implementation pointer. In informal testing (enumerating over a 10,000,000-item array ten times) the macro performed 49% faster than the standard idiom (averaging 3.6 million objects per second compared with 2.4 million per second). I tried other patterns but this one turned out to be the fastest.</li>
<li>If passed a nil pointer instead of a valid collection no iterations at all are performed, matching the behaviour of the standard idiom.</li>
<li>If passed an object which does not respond to the <tt>objectEnumerator</tt> selector (or <tt>keyEnumerator</tt> selector in the case of the <tt>WOKeyEnumerate</tt> macro) then an exception is raised, again matching the pattern of the standard idiom.</li>
<li>Note that the compiler C dialect must be set to C99 or GNU99 in order to use this macro because of the declaration of variables inside the <tt>for</tt> statement.</li>
<li>I have to use some casts between <tt>id</tt>, <tt>SEL</tt> and <tt>IMP</tt> types (in reality all just pointers) because C99 only allows a single declaration in the first expression of the <tt>for</tt> statement. In other words, "<tt>int i = 0, j = 0</tt>" (one declaration) is legal; "<tt>int i = 0, unsigned j = 0</tt>" (multiple declarations) is not. The casts are necessary to silence compiler warnings.</li>
<li>If you pass a collection object that responds to the <tt>objectEnumerator</tt> (or <tt>keyEnumerator</tt>) selector, but which returns an enumerator that does not respond to the <tt>nextObject</tt> selector then bad things will happen (in which case you probably deserve to crash for having passed such a collection).</li>
</ul>

<p>The one thing I don't like about this solution is that the macro looks like a function (name followed by parameters in parentheses) but behaves like a <tt>for</tt> statement (statement followed by expressions in parentheses followed by a block). That's probably while Michael named his macro <tt>foreach</tt> and swapped the order of the parameters (so that it reads like "<em>for each</em> item <em>in</em> collection". Nevertheless, I named my macros <tt>WOEnumerate</tt> and <tt>WOKeyEnumerate</tt> because I've made a habit of using the <tt>WO</tt> prefix to avoid namespace clashes, and I feel that the verb "enumerate" should appear in the macro.</p>

<p>Of course, I probably should have stuck to my conventions and used the form <tt>WO_ENUMERATE</tt> (I like to use all capitals for macros so that they scream out in the source code, "I am a macro and not a function!"; in general I make exceptions only where I want the macro to appear like an existing and closely-related Cocoa macro or function: for example, I have an <tt>WOStringFromBool</tt> macro that is intended to augment <tt>NSStringFromSelector</tt> and friends, and a <tt>WOAssert</tt> macro that's intended to supplement the <tt>NSAssert</tt> family). What was I thinking when I wrote these macros?... I suppose that suitable alternative names for the macros would be <tt>enumerate</tt> and <tt>keyenumerate</tt>; if used like that the macros would look like these when used:</p>

<pre>enumerate (fileName, fileArray)
    NSLog(@"Filename: %@", filename);</pre>

<p>So there you have it: three ways to improve upon the standard Objective-C enumeration idiom.</p>
...
<ol>
  <li>Firstly, I renamed the loop-local variables (<tt>enumerator</tt>, <tt>selector</tt> and <tt>method</tt>) to something more obscure so as to reduce the risk of namespace clashes.</li>
  <li>Later, used the GCC preprocessor macro concatentation operator (<tt>##</tt>) to further reduce the risk of namespace clashes by appending the value of <tt>object</tt> to those loop-local variable names. I use <tt>object</tt> because it has to be a simple scalar (unlike <tt>collection</tt> which could be something like "<tt>[thing message]</tt>").</li>
</ol>

<p>The sum effect of these changes is that you should be able to use the macros pretty much any (including nesting them) without feature of variable scoping issues or namespace clashes. If you want to check out the finished macros, now named <tt>WO_ENUMERATE</tt>, <tt>WO_KEY_ENUMERATE</tt> and <tt>WO_REVERSE_ENUMERATE</tt>,  see the <tt>WOEnumerate.h</tt> file in <a href="http://test.wincent.com/">WOTest</a>.</p>
</div></div>
