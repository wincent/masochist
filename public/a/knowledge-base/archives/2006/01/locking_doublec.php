...
<link rel="prev" href="http://www.wincent.com/a/knowledge-base/archives/2006/01/objectivec_enum.php" title="Objective-C enumeration macro" />

<link rel="next" href="http://www.wincent.com/a/knowledge-base/archives/2006/01/lightweight_iss.php" title="Lightweight issue tracking" />
...
<p align="right">
<a href="http://www.wincent.com/a/knowledge-base/archives/2006/01/objectivec_enum.php">&laquo; Objective-C enumeration macro</a> |

<a href="http://www.wincent.com/a/knowledge-base/">Main</a>
| <a href="http://www.wincent.com/a/knowledge-base/archives/2006/01/lightweight_iss.php">Lightweight issue tracking &raquo;</a>
...
<p>When you're doing multi-threaded programming it's often necessary to use locking to ensure that resources aren't being used in conflicting ways by different threads. There are three different approaches:</p>

<h4>Forget about locking, go for speeeeeed!!!!</h4>

<p>This one is potentially dangerous and even disastrous if the act of multiple threads attempting to perform the operation simultaneously will lead to problems.</p>
...
<p>This one is safe but not as fast because acquiring the lock can be time-consuming and the thread may have to block waiting for the resource.</p>
...
<p>Double-checked locking is often the best compromise between speed and safety. If the condition is fairly inexpensive to test (and it is in the case of the example because we're just checking the truth of a <tt>BOOL</tt> variable) then we can economically check the condition and acquire the lock if and only if we need to. We thus avoid a potentially expensive locking operation. But due to the possibility of a race condition we must then double-check the condition as shown below.</p>

<pre>OSMemoryBarrier(); // explained below
if (condition)
{
    @synchronized (self)
    {
...
<div id="a000188more"><div id="more">
<h4>Memory barriers</h4>

<p><a href="http://redshed.net/">Jonathan Rentzsch</a> pointed out to me that the <tt>@synchronized</tt> directive doesn't offer any documented guarantees about memory barriers and without these the double-checked locking method <a href="http://en.wikipedia.org/wiki/Double-checked_locking">may not be safe</a>. See <a href="http://www.aristeia.com/Papers/DDJ_Jul_Aug_2004_revised.pdf">this excellent paper</a> (PDF) for more information about possible pitfalls. It goes considerably further than the simplistic account offered <a href="http://www.cs.wustl.edu/~schmidt/PDF/DC-Locking.pdf">here in the paper</a> (PDF) that originally brought double-checked locking to my attention.</p>

<p>The use of the <a href="http://developer.apple.com/documentation/Darwin/Reference/ManPages/man3/OSMemoryBarrier.3.html">OSMemoryBarrier()</a> call is intended to address this risk but there is one problem: I've done some single-stepping into the function using GDB and although it appears to work fine on the G5 (ultimately relying on the <tt>isync</tt> and <tt>lwsync</tt> assembly instructions to implement the barrier) there are no guarantees that it will work on the Intel architecture (<a href="http://www.linuxjournal.com/article/8212">although I suspect it will</a> due to its stricter ordering properties).</p>

<p>I'm currently investigating this further, so in the absence of more concrete confirmation one way or the other, double-checked locking and the <tt>OSMemoryBarrier()</tt> call should probably be used with caution. Based on the information that I have been able to gather on the Internet so far I believe that the following directives represent the safest way to implement memory barriers on Mac OS X:</p>

<ul>
<li><strong>On PowerPC</strong>: <tt>sync</tt> for read and read/write memory barriers, <tt>eieio</tt> for write memory barriers.</li>
<li><strong>On PowerPC 64</strong>: <tt>lwsync</tt> for read memory barriers, <tt>sync</tt> for read/write and write memory barriers.</li>
<li><strong>On Intel</strong>: <tt>lock; addl</tt> for read and read/write memory barriers, and nothing at all for write memory barriers.</li>
<li><strong>On Intel with SSE support</strong> <em>(applies to any Intel machine that runs Mac OS X)</em>: <tt>lfence</tt> for read memory barriers, <tt>mfence</tt> for read/write memory barriers, and nothing at all for write memory barriers.</li>
</ul>

<p><a href="http://www.wincent.com/a/about/wincent/weblog/WOMemoryBarrier.h">Here's an include file</a> I've whipped up with some macros that implement these various combinations. It works for me, at least in my informal testing here (tested on both PowerPC and Intel CPUs).</p>

<p>If all of this seems a bit difficult <a href="http://www.aristeia.com/Papers/DDJ_Jul_Aug_2004_revised.pdf">the paper reference above</a> offers multiple alternative suggestions to the double-checked locking pattern, especially as it is applied to singletons. <a href="http://www.nwcpp.org/Downloads/2004/DCLP_notes.pdf">These notes by the same author</a> also offer some excellent insight into the problem.</p>

<h4>Notes</h4>

<p>You can often speed things up further by using a more granular lock. In this example I'm locking on <tt>self</tt> because it's quick and easy but you might be able to use another object for which there'll be less contention (for example, a specially created <tt>NSLock</tt> object) and which will lead to fewer (or shorter) blocking situations.</p>

<p>This may be self-evident to some but I think it's nice to state it explicitly: the compiler itself is clever enough to handle things like the following example. You can jump out of the scope of a <tt>synchronized</tt> block in a number of ways (in this case by way of an early <tt>return</tt>); it's not necessary to "fall off the end":</p>
...
<p>The other thing to bear in mind is that the choice between normal and double-checked locking will depend on a number of factors:</p>

<p><em>Is testing the condition an inexpensive operation compared to that of acquiring a lock?</em> If so then double-checked locking is favourable.</p>

<p><em>Is contention for the lock likely to be high?</em> That's another factor which makes double-checked locking desirable, because it avoids unnecessary contention for the lock.</p>

<p><em>How often is the condition likely to be true?</em> If the condition tends to be true more often than not then double-checked locking is probably a bad idea, because in most situations you'll end up having to acquire the lock anyway and will have done the double-check for nothing.</p>

<p>So basically you'll need to think about the specific details of your situation and choose the method that best balances speed and safety accordingly.</p>
</div></div>
