...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Knowledge base: WordPress updates via Subversion</title>
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.com/a/knowledge-base/archives/2006/06/intel_imac_setu.php"
            >&laquo; Intel iMac set-up notes</a
          >
          |
          ...
          <a
            href="https://wincent.com/a/knowledge-base/archives/2006/06/wordpress_secur.php"
            >WordPress security tip &raquo;</a
          >
        </p>

        <h2>June 02, 2006</h2>

        <h3>WordPress updates via Subversion</h3>
        ...
        <p>
          ...
          Subversion to pull down updates and merge them automatically. This is
          a technique already used by successful projects like
        ...
        <div id="a000344more">
          <div id="more">
            <p>
              We start off by doing a checkout of the latest release in the 2.0
              branch (2.0.3) at the time of writing:
            </p>
...
sudo svn co http://svn.automattic.com/wordpress/branches/2.0/ .</pre
            ...
            <p>
              Then it's time to do a <tt>diff</tt> to see the differences
              between the freshly-checked-out copy of WordPress 2.0.3 and the
              installed, customized copy of 2.0.2 already on the server. It's
              necessary to manually merge in these changes, but only on this
              initial switch to Subversion. Once the first merge is done, future
              upgrades are as easy as running <tt>svn up</tt>.
            </p>

            <pre>diff -r path-to-checked-out-copy path-to-installed-copy</pre>

            <p>
              Concretely, in my case this meant that I had to use the following
              paths:
            </p>
            ...
            <p>
              Given that most of my existing WordPress install was not
              customized I found that it was more useful to do a non-recursive
              <tt>diff</tt> and then examine the appropriate subdirectories
              manually:
            </p>
            ...
            <p>
              The <tt>diff</tt> output immediately indicated to me a couple of
              missing files that I would need to copy over from my old install
              to the new copy:
            </p>
...
sudo cp public_html/wp/wp-config.php wp/</pre
            ...
            <p>
              There were a couple of common subdirectories that I could skip
              over and exclude from consideration because I've applied no
              customizations to the WordPress code itself:
            </p>
...
Common subdirectories: wp/wp-includes and public_html/wp/wp-includes</pre
            ...
            <p>
              Likewise there was a <tt>wp-images</tt> folder in the old install
              that is evidently not present in the latest version of WordPress,
              so I didn't bother to copy it over:
            </p>

            <pre>Only in public_html/wp: wp-images</pre>

            <p>
              The key directory that I needed to examine carefully was the
              <tt>wp-content</tt> directory:
            </p>

            <pre>
Common subdirectories: wp/wp-content and public_html/wp/wp-content</pre
            >
...
sudo cp -R public_html/wp/wp-content/uploads wp/wp-content/</pre
            ...
            <p>
              There were a couple of common subdirectories that needed closer
              attention:
            </p>
...
Common subdirectories: wp/wp-content/themes and public_html/wp/wp-content/themes</pre
            ...
            <pre>
diff wp/wp-content/plugins public_html/wp/wp-content/plugins</pre
            >
            ...
            <pre>
sudo cp -R public_html/wp/wp-content/plugins/text-control wp/wp-content/plugins/</pre
            >

            <p>
              Likewise I did a non-recursive <tt>diff</tt> of the
              <tt>themes</tt> subdirectory:
            </p>

            <pre>
diff wp/wp-content/themes public_html/wp/wp-content/themes</pre
            >
            ...
            <pre>
sudo cp -R public_html/wp/wp-content/themes/hemingway wp/wp-content/themes/</pre
            >

            <p>
              It was then time to set the permissions and ownership on the new
              install (especially important seeing as I am running under PHP
              safe mode which is much more sensitive to permissions):
            </p>
...
sudo chown -R apache:apache *</pre
            ...
            <p>
              It was then time to make the actual switch from 2.0.2 to 2.0.3 on
              the live site. This consisted of five steps:
            </p>
            ...
            <ol>
              ...
              <li>
                Move the old directory out of the way and the new one into place
                (in my case achieved with
                <tt>sudo mv public_html/wp ./wp-old</tt> followed by
                <tt>sudo mv wp public_html/</tt>).
              </li>
              <li>
                Visit http://example.com/wp/wp-admin/ and follow the prompts to
            ...
            <p>
              And that's all there is to it. To see differences between the
              installed version of WordPress and the last version that was
              checked out it's a simple matter of doing an
              <tt>svn status</tt> from inside the installation directory. On my
              installation I see the following output:
            </p>
...
Performing status on external item at 'wp-content/plugins/akismet'</pre>
            ...
            <p>
              As you can see, the changes correspond to those that I made during
              my manual merge. For more information on the
              <tt>svn status</tt> command type <tt>svn help status</tt>. For
              (much) more detail on the differences you can do an
              <tt>svn diff</tt>.
            </p>

            <p>
              Now, when the next version of WordPress comes out, all you have to
              do is run an <tt>sudo svn update</tt>. Given the sensitivity of
              PHP Safe Mode you should also make sure no new root-owned files
...
find . -user root</pre
            ...
            <p>
              Any root-owned files will need to be appropriately modified using
...
sudo chown user:user file</pre
            ...
            <p>
              More
              <a href="http://codex.wordpress.org/Using_Subversion"
                >information on using Subversion with WordPress here</a
              >.
            </p>
          </div>
        </div>

        <p class="posted">Posted by wincent at June 2, 2006 06:41 PM</p>
      </div>
    </div>
