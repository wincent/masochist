...
<link rel="prev" href="https://wincent.com/a/knowledge-base/archives/2004/12/upgrading_to_su.php" title="Upgrading to Subversion 1.1.2" />

<link rel="next" href="https://wincent.com/a/knowledge-base/archives/2005/04/information_for.php" title="Information for beta testers" />
...
<p align="right">
<a href="https://wincent.com/a/knowledge-base/archives/2004/12/upgrading_to_su.php">&laquo; Upgrading to Subversion 1.1.2</a> |

<a href="https://wincent.com/a/knowledge-base/">Main</a>
| <a href="https://wincent.com/a/knowledge-base/archives/2005/04/information_for.php">Information for beta testers &raquo;</a>

</p>

<h2>April 09, 2005</h2>

<h3>Upgrading to Subversion 1.1.4</h3>

<p><a href="http://subversion.tigris.org/">Subversion</a> 1.1.4 came out a few days ago so I decided to take the opportunity to upgrade my existing installation and also update my existing <a href="http://www.sleepycat.com/download/db/index.shtml">BerkleyDB</a> install to the latest release (4.3.27). Unlike <a href="https://wincent.com/a/knowledge-base/archives/2004/12/upgrading_to_su.php">earlier versions</a> this Subversion release works fine with the BerkleyDB 4.3 series.</p>

<p>As per usual, these instructions assume that you have the Xcode Tools installed and they use <a href="https://wincent.com/a/knowledge-base/archives/2004/11/building_wget_f.php">wget</a> to grab the source. If you don't have wget you could use curl, which is bundled with Mac OS X (type "man curl" in the terminal for more details). If you want to perform a new install of Subversion rather than just upgrade you should see the <a href="https://wincent.com/a/knowledge-base/archives/2004/11/building_subver.php">original article</a> in which I performed the installation.</p>

<div id="a000053more"><div id="more">
<p>Because I'm updating the backend (the database) I had to do a dump of my repository, perform the upgrade and then import the dumps into a new repository as described <a href="http://svnbook.red-bean.com/en/1.1/ch05s03.html#svn-ch-5-sect-3.5">here</a>. For example, to dump my <a href="http://synergy.wincent.com/">Synergy</a> repository I used a bunch of commands like this (where ~/Developer/svnrep/ is the folder where I keep all my repositories):</p>

<pre># stop anything that might be accessing the repository
sudo /usr/local/apache2/bin/apachectl stop
<br />
# create a temporary working space for the dumps
mkdir dumps && cd dumps
<br />
# perform the actual dump
sudo svnadmin dump ~/Developer/svnrep/Synergy &gt; \
                   Synergy.dump
<br />
# move the repository out of the way
sudo mv ~/Developer/svnrep/Synergy \
        ~/Developer/svnrep/Synergy.old</pre>

<p>Once I had done that for all repositories I was free to continue:</p>

<pre># install BerkleyDB
...
<p>Now I was ready to recreate my old repositories and insert the contents back in:</p>

<pre># go back to where you put the dumps
cd dumps
<br />
# create a new empty repository
sudo svnadmin create ~/Developer/svnrep/Synergy
sudo chown -R svn:svn ~/Developer/svnrep/Synergy
<br />
# restore its contents from the dump
sudo svnadmin load ~/Developer/svnrep/Synergy &lt; \
                   Synergy.dump
sudo chown -R svn:svn ~/Developer/svnrep/Synergy
<br />
...
<p>And that's all for, until next time... (looks like we can expect a <a href="http://subversion.tigris.org/svn_1.2_releasenotes.html">1.2 release</a> soon).</p>
</div></div>

<p class="posted">Posted by wincent at April  9, 2005 06:52 AM</p>
