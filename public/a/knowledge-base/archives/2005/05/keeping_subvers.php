...
<title>Knowledge base: Keeping Subversion alive and other notes on the update to Mac OS X 10.4</title>
...
<link rel="prev" href="http://www.wincent.com/a/knowledge-base/archives/2005/04/information_for.php" title="Information for beta testers" />

<link rel="next" href="http://www.wincent.com/a/knowledge-base/archives/2005/05/mac_os_x_tiger.php" title="Mac OS X Tiger: Initial response" />
...
<p align="right">
<a href="http://www.wincent.com/a/knowledge-base/archives/2005/04/information_for.php">&laquo; Information for beta testers</a> |

<a href="http://www.wincent.com/a/knowledge-base/">Main</a>
| <a href="http://www.wincent.com/a/knowledge-base/archives/2005/05/mac_os_x_tiger.php">Mac OS X Tiger: Initial response &raquo;</a>

</p>

<h2>May 16, 2005</h2>

<h3>Keeping Subversion alive and other notes on the update to Mac OS X 10.4</h3>

<p>It's now been a few weeks since I installed <a href="http://www.apple.com/macosx/">Tiger</a> and I wanted to post some notes on the move from 10.3 to 10.4. These contain a lot of material which is specific to my own set-up (hard coded paths, pre-existing folders etc) but they may be of interest to people who have read my previous articles on installing <a href="http://www.wincent.com/a/knowledge-base/archives/2005/04/upgrading_to_su_1.php#more">Subversion</a> and some other tools on this site.</p>

<div id="a000068more"><div id="more">
<p>I first tried an upgrade install and had to perform several hard resets due to a conflict between <a href="http://usboverdrive.com/">USB Overdrive</a> and my <a href="http://apc.com/">APC UPS</a> (I suggest you download the latest version of USB Overdrive; it fixes the problem). Of course, at that point I didn't know the cause of the conflict. I tried again, doing an archive install instead, which meant that I then had to reinstall a lot of software...</p>
...
<p>As wget 1.10 is just around the corner I decided to install from the latest CVS sources again (see <a href="http://www.wincent.com/a/knowledge-base/archives/2004/11/building_wget_f.php">previous article</a>).</p>
...
<p>Nothing much has changed since my <a href="http://www.wincent.com/a/knowledge-base/archives/2004/12/building_pstree.php">original article</a>.</p>
...
<p>Again, nothing much has changed since <a href="http://www.wincent.com/a/knowledge-base/archives/2004/11/building_pwgen.php">last time</a>, so I used the source tree that I originally downloaded:</p>

<pre># use previously downloaded
...
<p>Nothing much new to report since <a href="http://www.wincent.com/a/knowledge-base/archives/2004/11/building_eeaee.php">last time</a>. Once again I decided to use the same source trees:</p>
...
<p>This one turned out to be unnecessary. In the end I ended up converting my Subversion repositories to use the new FSFS (filesystem) back-end.</p>

<pre># use previously downloaded source
...
<pre># use last downloaded version
...
<p>I made the following changes to /usr/local/apache2/conf/httpd.conf. Your local setup will no doubt be different, especially the directory paths. This configuration also depends on the svn user and group that I created when I <a href="http://www.wincent.com/a/knowledge-base/archives/2004/11/building_subver.php">first installed</a> Subversion on this machine (which survived even though I did an archive install because users and groups in the NetInfo database are preserved).</p>
...
<pre>sudo mkdir -p /usr/local/apache2/var/dav
sudo chown svn:svn /usr/local/apache2/var/dav</pre>

<p>And set up startup item:</p>

<pre>sudo cp -R /System/Library/StartupItems/Apache \
/Library/StartupItems/Apache2</pre>

<p>Inside the newly created Apache2 folder I made these changes to the file Apache and renamed it to Apache2:</p>
...
<p>/etc/hostconfig already contained the line WEBSERVER2=-YES- from the previous install. On rebooting the Apache2 webserver will automatically run thanks to the startup item.</p>

<h4>Converting the Subversion repositories to the FSFS backend</h4>

<p>I then ran into an unfortunate situation. I was getting commit errors due to database version mismatch. It appears the even though my last version of svn was linked against BerkeleyDB 4.3, my previous Apache installed had been linked against 4.2 and so I couldn't touch my repository.</p>

<p>My "solution" was to use 10-day old dump and recreate repositories, this time using FSFS. I suppose that I could have tried recompiling everything all over again with an older version of BerkeleyDB, but I just didn't feel up to it. This underlines the need to always, always keep good backups, and if you're running Subversion make regular repository dumps. I think future upgrades will be a lot more painless now that I've switched to FSFS:</p>
...
<p>Seeing as have done some Perl work lately I decide to add the following to the autoprops section of my svn configuration file (~/.subversion/config):</p>

<pre>*.pm = svn:keywords=Id</pre>

<p>I also decided to put an end to the endless commit hassles I've been suffering thanks to Interface Builder creating backup nib files which end with "~.nib". I uncommented:</p>

<pre>global-ignores = *.o *.lo *.la #*# .*.rej *.rej .*~ *~ .#* .DS_Store</pre>
