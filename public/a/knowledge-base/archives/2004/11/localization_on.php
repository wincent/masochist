...
<link rel="prev" href="https://wincent.com/a/knowledge-base/archives/2004/11/cleaner_html_us.php" title="Cleaner HTML using CSS" />

<link rel="next" href="https://wincent.com/a/knowledge-base/archives/2004/11/costs_of_runnin.php" title="Costs of running a software small business" />
...
<p align="right">
<a href="https://wincent.com/a/knowledge-base/archives/2004/11/cleaner_html_us.php">&laquo; Cleaner HTML using CSS</a> |

<a href="https://wincent.com/a/knowledge-base/">Main</a>
| <a href="https://wincent.com/a/knowledge-base/archives/2004/11/costs_of_runnin.php">Costs of running a software small business &raquo;</a>
...
<p>Mac OS X has a fully-fledged and elegant localization infrastructure that enables software to be translated into multiple languages and shipped as a single convenient package to end users. In this article I don't rehash the material on localization that is already available on the web. I am specifically writing about what you would need to do to produce a localization of a <a href="https://wincent.com/">Wincent product</a>.</p>

<div id="a000020more"><div id="more">
<h3>Background</h3>

<p>This article assumes some prior knowledge, so if you don't yet know about the following topics you should probably do some background reading and make sure you have the appropriate tools first:</p>

<ul>
  <li>General reading on internationalization:
    <ul>
      <li>Apple's <a href="http://developer.apple.com/intl/">International Technologies page</a>.</li>
      <li>Apple's <a href="http://developer.apple.com/intl/localization/">Localization on Mac OS X page</a>.</li>
      <li>Apple's <a href="http://developer.apple.com/documentation/MacOSX/Conceptual/SystemOverview/International/chapter_10_section_2.html">Mac OS X System Overview: Internationalizing Your Application</a>.</li>
      <li>StepWise article, <a href="http://www.stepwise.com/Articles/Technical/2003-04-03.01.html">Internationalizing Cocoa applications</a> by mmalcolm Crawford.</li>
      <li><a href="http://www.stone.com/The_Cocoa_Files/Internationalize_Your_App.html">Internationalizing Cocoa Applications</a> by Andrew Stone.</li>
    </ul>
  </li>
  <li>Localization tools and resources (see Apple's <a href="http://developer.apple.com/intl/localization/tools.html">Localization tools page</a> to be sure that you get the latest versions; these links below last updated 8 April 2006):
    <ul>
      <li><a href="ftp://ftp.apple.com/developer/Tool_Chest/Localization_Tools/ADViewer_2.1.dmg">ADViewer 2.1 disk image</a>: the text editor recommended by Apple for use in localization.</li>
      <li><a href="ftp://ftp.apple.com/developer/Tool_Chest/Localization_Tools/AppleGlot/AppleGlot_3.2.dmg">AppleGlot 3.2 disk image</a>.</li>
      <li><a href="ftp://ftp.apple.com/developer/Tool_Chest/Localization_Tools/AppleGlot/AppleGlot_3.2_UsersGuide.pdf">AppleGlot 3.2 User Guide</a> (PDF).</li>
      <li><a href="ftp://ftp.apple.com/developer/Tool_Chest/Localization_Tools/Mac_OS_X_Glossaries">AppleGlot glossaries</a> (FTP site).</li>
    </ul>
  </li>
</ul>
...
<p>Whenever I prepare a final build for shipping, as a final step in the build process I use a collection of tools, including Apple's <a href="http://developer.apple.com/documentation/Darwin/Reference/ManPages/man1/genstrings.1.html">genstrings</a> and <a href="http://developer.apple.com/documentation/Darwin/Reference/ManPages/man1/nibtool.1.html">nibtool</a> utilities, <a href="http://www.mani.de/nibtrans.sh.gz">nibtrans</a> by Manfred Schwind and <a href="https://wincent.com/a/products/wincent-strings-util/">a modified version</a> of <a href="ftp://ftp.omnigroup.com/pub/localization/">stringsUtil</a> by the <a href="http://www.omnigroup.com/">OmniGroup</a>, to produce a localization package that is made available for download on the corresponding product's download page.</p>

<p>Before you start work on a localization you should <a href="https://wincent.com/a/contact/mail/">contact me</a> to let me know that you are going to start work on it. Sometimes more than one person steps forward to do a localization, so it's useful to coordinate your efforts with other people and minimize duplication of effort.</p>

<p>By default, every localization pack includes one or more English localizable project folders ("English.lproj" or "en.lproj") containing the unlocalized, English-language versions of the program resources. Sometimes there is more than one such folder because a given product may actually be comprised of various components, each with its own "English.lproj" or "en.lproj" folder. If you are localizing a product into a target language for the first time, make a copy of the English resources, rename the containing folders appropriately (for example, "Spanish.lproj", "Japanese.lproj" and so on), and perform the localization. You must localize every file in every component otherwise the localization will be incomplete.</p>

<p>In cases where a localization already exists for a given product, the build process will merge the new, unlocalized portions of the project into the existing localized resources. In this way you don't have to begin the localization process all over again from the beginning. Rather, you can do an incremental localization, translating only the new (unlocalized) additions. Where pre-existing localizations exist, the merged lproj folders awaiting incremental localization will be included in the downloadable localization pack. Look at each and every file and make sure that there are no new strings that need to be translated. When you send back the updated localization you should only send files to which you actually made changes; if you did not make any changes to a file then do not send it back (the easiest way to do this is probably to simply delete those files to which you did not make any changes). Likewise, if you updated the Japanese localization do not send back the other localizations as well.</p>

<p>As a general rule localization packs contain string files and not nib files; this is to streamline the process and make incremental localization more straightforward. It is also necessary in some cases where the nib files depend on private frameworks which are not included in the standard Mac OS X install and which Interface Builder will refuse to open. It is important that when you provide translations that you pick wordings that are of approximately the same length as their untranslated counterparts. If the translations are too lengthy then they will not fit within the geometry of the user interface. I can make minor adjustments to the interface geometry, but in general it is best for the purposes of consistency if translations can be chosen which obviate the need to do this.</p>

<p>If you do decide to do a localization please make sure that it is complete. Merging localizations is a time-consuming process that takes a lot of work, so it is not very efficient to integrate partial localizations (and then updates to those localizations). This is why I ask that when you do an updated localization that you only send back the files which you have actually modified. If you do not have the time to do a full localization of every localizable file then you should not go ahead with the project. Fully localizing the project also means localizing the InfoPlist.strings files which are often overlooked; for example, the English words for "copyright" and "version" should be translated.</p>

<h3>Summary</h3>

<ul>
  <li>Use a Unicode-aware text editor, such as ADViewer.</li>
  <li>Save ".strings" files with UTF-16 encoding, as per the Apple documentation.</li>
  <li>Save ".plist" files with UTF-8 encoding, as per the encoding attribute at the head of the file.</li>
  <li>Double check the encoding before you send back the localized files.</li>
  <li>Keep your translations roughly the same length as the original text.</li>
  <li>If you can, get a friend to double-check your translations.</li>
  <li>Localize all the files, including InfoPlist.strings files.</li>
  <li>Only submit complete localizations, not partial ones.</li>
  <li>If updating an existing localization, only send back the files which you actually modified.</li>
  <li>Send only the lproj folders for the specific language you are working on; do not send back the lproj folders for the other languages.</li>
</ul>
</div></div>
