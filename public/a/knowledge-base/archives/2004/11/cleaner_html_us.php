...
<link rel="next" href="https://wincent.com/a/knowledge-base/archives/2004/11/localization_on.php" title="Localization on Mac OS X" />
...
| <a href="https://wincent.com/a/knowledge-base/archives/2004/11/localization_on.php">Localization on Mac OS X &raquo;</a>
...
<p><a href="http://www.w3.org/Style/CSS/">CSS</a>, or Cascading Style Sheets, have been around for a while now, but I don't think I've ever truly "gotten" them, until this week that is... Years ago, back in the days of Netscape Navigator 4, I had a brief play with them. They promised to deliver a greater degree of control over the final appearance of the page. In fact, many even argued that you could throw out tables and use CSS to do layout (examples <a href="http://glish.com/css/">here</a> and <a href="http://www.bluerobot.com/web/layouts/">here</a>).</p>

<p>The trouble was, <a href="http://www.microsoft.com/">one company in particular</a> didn't support CSS very well, requiring all sorts of <a href="http://www.positioniseverything.net/articles/hollyhack.html">hacks</a> to get it to display correctly. This seemed like a travesty in the 21st century with <a href="http://www.mozilla.org/">Mozilla</a> and others championing the cause of standards-compliance. It seemed that the ugly hacks required to get CSS to work on all browsers were not much better than the complicated nested tables that they replaced. Over the years I've periodically checked back to see what the state of play was in the realm of CSS. In general it was something that I tacked on to my pages where I thought it wouldn't break things, but it didn't really revolutionize the way I went about designing my pages.</p>

<p>All that seems to be changing. Thanks to <a href="http://www.apple.com/safari/">Safari</a> and it's WebKit engine (also used in <a href="http://www.omnigroup.com/applications/omniweb/">OmniWeb</a>), and the host of browsers using the Mozilla Gecko engine (such as <a href="http://www.mozilla.org/products/camino/">Camino</a>, <a href="http://www.mozilla.org/products/firefox/">FireFox</a> and  of course <a href="http://www.mozilla.org/products/mozilla1.x/">Mozilla itself</a>), I can now write standards-compliant web pages and be confident that around 99% of my intended audience will see them as I intended them to be seen. <a href="http://www.opera.com/">Opera</a> also does a great job of compliance, and even Microsoft's browsers tend to display simple pages more or less correctly, even if they don't end up fully supporting every little touch and feature that you've added.</p>

<p>I'm a Mac software developer, which puts me in a very special position. I know that most of my visitors will be using Macs, and they'll see what they should be seeing. Those who visit using Windows machines will see something reasonably close, because my pages are fairly clean and simple.</p>

<p>It's a great time to be a web designer.</p>

<p>With that lengthy preamble out the way, the rest of this article will talk about what I think is the real strength of well-prepared CSS: it's ability to help you write clean HTML.</p>

<div id="a000018more"><div id="more">
<h3>How to use (and not use) CSS</h3>

<p>It's only in these last few days that I've finally "gotten" CSS. It's just like Object-Oriented Programming (OOP), in a way. In OOP, you minimize redundancy and maximize code re-use by moving common elements "up" the hierarchy. With CSS, you do the same: you move common styling information "up" (from the HTML into CSS styles), and in fact you can even move it right out into a separate file. That's what I've done with the current design for <a href="https://wincent.com/">wincent.com</a>; there's <a href="https://wincent.com/a/styles.css">only one style sheet</a> on the whole website, and every other page makes use of it.</p>

<p>There's a clear advantage there in terms of style re-use. Change that one style sheet and everything else gets updated automatically.</p>

<p>But it wasn't until I read this <a href="http://css.maxdesign.com.au/selectutorial/">excellent tutorial on CSS selectors</a> by <a href="http://www.maxdesign.com.au/presentation/">Max Design</a> that I realized there was another benefit that I hadn't seen before. I suddenly realized how much cleaner my HTML could become if I just started using selectors wisely. Use selectors wisely and you'll not only move more styling information "up" and out of your HTML, but you'll get the corollary benefit that your HTML source is infinitely more readable and better looking.</p>

<p>The key thing to bear in mind is this: <em>Override existing HTML elements to produce the look you want</em>.</p>

<p>You can also think of this conversely, as: <em>Don't write special classes for each and every type of element</em>.</p>

<p>Applied judiciously, these two little ideas will help you to write shorter, more compact style sheets, and much better looking HTML. Let's demonstrate this by looking at an example of what not to do. Imagine a web page that has the following content:</p>
...
<p>This is an atrocious example of CSS gone wrong. You can imagine the entire site peppered with hundreds of different styles, and the accompanying, gargantuan, style sheet. It's true that the author has moved lots of HTML attributes out of the HTML, but it's been only to replace them with a myriad of CSS class names. All of this looks very difficult to maintain; the class names are hard to remember and easy to misspell. There are too many of them.</p>

<p>With intelligent use of selectors the same examples could be written like this:</p>
...
<p>In keeping with what I said about moving things "up" the hierarchy, you can use selectors to define the look of things in higher-level elements of the page, like the "body" element, or something fundamental like the "p" element. By doing this you minimize the amount of repetitious styles you need to define in your style sheet. And you're not forfeiting any control; notice that even though you use the same ("h3") tag in two different places, you can still independently control the appearance of each instance.</p>

<p>Selectors are incredibly powerful things. You're not just limited to saying "class X should look like this", nor even things like in our example above, "element Y should look like this whenever it appears inside Z". You can define targets like "element A should look like this whenever it appears inside a B, which appears inside a C, which appears inside a D", or "element E should look like this whenever it is immediately preceded by element F", or "element G should look like this only when its immediate parent is element H", and so forth. The level of control possible is truly impressive. It's this kind of control that makes the dynamic menus that you see above in the navigation bar possible (see <a href="http://www.meyerweb.com/eric/css/edge/menus/demo.html">this article</a> by Eric Meyer for a description of the idea that's behind the effect); all of this is down without any JavaScript whatsoever, and it's pretty much guaranteed to work in your browser (you're not using Internet Explorer, are you?...)	</p>

<h4>Conclusion</h4>

<p>The core thing to take from this article is that piece of two-side advice:</p>

<ul>
  <li>Override existing HTML elements to produce the look you want.</li>
  <li>Don't write special classes for each and every type of element.</li>
</ul>

<p>And <em>do</em> visit the <a href="http://css.maxdesign.com.au/selectutorial/">select tutorial</a> referred to above so you can learn about the all the ways in which you can use selectors to write better CSS.</p>
</div></div>
