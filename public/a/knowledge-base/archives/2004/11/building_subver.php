...
<p>Anyway, for the record, here is how to build Berkley DB 4.3.21. This and the older version, 4.2.52, can happily coexist on the same system.</p>

<pre>tar xzvf db-4.3.21.tar.gz 
cd db-4.3.21/build_unix/
../dist/configure
make
sudo make install</pre>

<p>Now here's how to build 4.2.52, after you've obtained the patches from <a href="http://www.sleepycat.com/update/4.2.52/patch.4.2.52.html">here</a>. This assumes you've expanded the source archive, downloaded the patches, and copied them into the directory that appears when you expand the archive.</p>

<pre>patch -p0 &lt; patch.4.2.52.1
...
<p>Now onto <a href="http://httpd.apache.org/">Apache 2</a>. I downloaded <a href="http://apache.rediris.es/httpd/httpd-2.0.52.tar.gz">2.0.52</a>, which is the latest at the time of writing. Note that I build Apache with SSL support in case I ever want to put the repository out on the Internet. For now I only intend to use the server "in house", behind a firewall (in fact, I'll be connecting to it on "localhost") so I haven't set up SSL just yet.</p>

<pre>tar zxvf httpd-2.0.52.tar.gz 
cd httpd-2.0.52
./buildconf 
...
<p>With these prerequisites in place I moved onto Subversion itself:</p>
...
<p>I then wanted to make Apache 2 start up at boot time, but without interfering with the existing Apache installed by default on Mac OS X. I took the standard Apache startup item, copied it, and made appropriate customizations. The following steps show the copying, but I haven't shown the customizations.</p>

<pre>cd /System/Library/StartupItems/
cp -r Apache /Library/StartupItems/Apache2
cd /Library/StartupItems/Apache2</pre>

<p>Once customized you will need to add a line like "WEBSERVER2=-YES-" (I used "WEBSERVER2" to distinguish the Apache 2 webserver from the default Apache 1 server which appears as "WEBSERVER") to the /etc/hostconfig file (which needs root privileges).</p>

<p>I then made some changes to the /usr/local/apache2/conf/httpd.conf configuration file. To avoid clashes with the default server, I changed the "Listen 80" directive to read "Listen 8080". I am not sure what the standard port to use for this purpose is, because I've seen 8080, 8800 and 8008 recommended in different places on the web. The truth is it doesn't have a lot of importance.</p>
...
<p>And edited the "ServerAdmin" directive to match my email address. Finally, I added some lines to the bottom of the file. As you can see, my repository is for now located at "/Users/wincent/Developer/svnrep":</p>

<pre># prevent normal access; this server is for Subversion only
...
<p>I then needed to set up the svn user and group. I found out about <a href="http://www.mcelhearn.com/article.php?story=2004110211244242&#38;mode=print">neat way to do this</a> from the command line: </p>

<pre>echo 'svn::200:200::0:0:Subversion:/var/empty:/usr/bin/false' |\
...
<p>Finally, I had to create the "/usr/local/apache2/var/dav/" and ensure that the svn user/group could write to it:</p>

<pre>sudo mkdir -p /usr/local/apache2/var/dav
chown svn:svn /usr/local/apache2/var/dav</pre>

<p>Things work just like my old Subversion repository, except that now I access things using "http://localhost:8080/svnrep" URLs instead of "file:///usr/local/svnrep" URLs. Apache is the arbiter of access to the repository itself, the parent directory is owned by the svn user and group, so I can't write to the repository by mistake if I use a file:/// URL. Xcode integration continues to work fine with the new set-up.</p>
</div></div>
