...
<link rel="prev" href="http://www.wincent.com/a/knowledge-base/archives/2004/12/building_pstree.php" title="Building pstree 2.26 on Mac OS X 10.3.6" />

<link rel="next" href="http://www.wincent.com/a/knowledge-base/archives/2004/12/upgrading_to_su.php" title="Upgrading to Subversion 1.1.2" />
...
<body>
  ...
  <div id="breadcrumbs">
    <p><a href="http://www.wincent.com/">Main</a>&nbsp;&gt;&nbsp;<a href="../../../">Knowledge Base</a>&nbsp;&gt;&nbsp;<a href="../../">Archives</a>&nbsp;&gt;&nbsp;<a href="../">2004</a>&nbsp;&gt;&nbsp;12    </p>
  </div>
...
<p align="right">
<a href="http://www.wincent.com/a/knowledge-base/archives/2004/12/building_pstree.php">&laquo; Building pstree 2.26 on Mac OS X 10.3.6</a> |

<a href="http://www.wincent.com/a/knowledge-base/">Main</a>
| <a href="http://www.wincent.com/a/knowledge-base/archives/2004/12/upgrading_to_su.php">Upgrading to Subversion 1.1.2 &raquo;</a>
...
<p>I often use Objective-C's ability to send messages to nil to help make my source code more readable. But today I discovered a bug in <a href="http://www.wincent.com/a/products/winswitch/">WinSwitch</a> that grew out of this habit and caught me off guard. This article describes the bug in the hopes that others won't fall prey to it in the future.</p>

<div id="a000039more"><div id="more">
<h4>The idea</h4>

<p>Basically, the idea is that when you send a message to nil, the result will be nil. This allows you to write things like the following:</p>
...
<p>I noticed that WinSwitch was showing the user icons in the menu bar at the wrong size instead of the default whenever no preferences were set. I peppered my code with some NSLog statements to find out what was actually happening, and made a test project to isolate the problem. Here is an example of code to reproduce the fault:</p>
...
<p>In the example above I purposely provide a nil dictionary just to demonstrate what happens when the user preferences aren't set. I would expect aFloat to be 0.0, but it wasn't; it was returning insane values, and not always the same ones! At first I thought I had discovered a bug in GCC or in the Objective-C runtime itself. Then I found <a href="http://developer.apple.com/documentation/Cocoa/Conceptual/ObjectiveC/LanguageOverview/chapter_3_section_3.html">this page</a> in the Apple documentation, which says:</p>

<blockquote>"A message to nil also is valid, as long as the message returns an object; if it does, a message sent to nil returns nil. If the message sent to nil returns anything other than an object, the return value is undefined."</blockquote>

<p>And there is your answer. The "floatValue" method returned something other than an object, so the return value was undefined. This one caught me by surprise, because my experience with other kinds of non-object return values (such as ints and BOOLs) had always behaved exactly as you would expect. For example:</p>

<pre>NSObject *nothing = nil;
int anInt  = [nothing intValue];  // returns 0
BOOL aBOOL = [nothing boolValue]; // returns NO (0)</pre>

<p>This makes perfect sense to me, because a message to nil is really a message to (id)0. It makes sense that it should return 0, coerced appropriately: 0 for ints, 0.0 floats, NO for BOOLs, and so forth. But it is not so, and the documentation does make note of it. Now I have to look back over all the code I've ever written and try to find places where I rely on Objective-C's ability to send messages to nil, and make sure that I'm only using that behaviour in cases where an object gets returned. Not an easy task!</p>
</div></div>
