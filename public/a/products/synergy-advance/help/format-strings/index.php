...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Synergy Advance help: format strings</title>
  ...
  <div id="breadcrumbs">
    <p><a href="http://www.wincent.com/">Main</a>&nbsp;&gt;&nbsp;<a href="../../../">Products</a>&nbsp;&gt;&nbsp;<a href="../../">Synergy Advance</a>&nbsp;&gt;&nbsp;<a href="../">Help</a>&nbsp;&gt;&nbsp;Format Strings    </p>
  </div>
    <div id="container">
      <div id="left">
        <div class="sidebar">
      ...
      <div id="center">
        <div class="content">
          ...
          <h3>Synergy Advance help: format strings</h3>
          
          <h4>Basics</h4>
          
          <p>Synergy Advance makes use of user-definable <em>format strings</em> in a number of places to allow a high degree of customization of text. A "string" is merely a string of textual characters. A "format string" is a special kind of string that describes the <em>format</em> that a string should take. Format strings are commonly used in programming because they are a compact way of representing textual patterns. Traditionally the percent symbol (%) has been used to introduce a format marker; the character that immediately follows the percent symbol indicates what kind of information should be inserted in the string.</p>
          
          <p>The following is an example of a simple format string. The "%t" is a format marker because it is introduced by a percent symbol; it indicates that the current track title should be inserted in the string at that point, replacing the format marker. The "%a" is also a format marker and it indicates that the artist of the current track should be substituted at that point:</p>
          
          <pre>Track title: %t by %a</pre>
          
          <p>If the current track title is "Se esfuma tu amor" and the artist is "Marc Anthony", then the format string will be translated into the following at runtime:</p>
          
          <pre>Track title: Se esfuma tu amor by Marc Anthony</pre>
          
          <p>Format strings are case sensitive. If you change "%t" to "%T", or "%a" to "%A" in the example above then no substitution will occur and Synergy Advance will print a warning to the console indicating that "%T" and "%A" are not valid format markers (unless you've turned the <a href="../preferences/advanced/#logging">logging level</a> down to zero in the <a href="../preferences/advanced/">Advanced preferences</a>).</p>
          
          <p>As of version 0.5, Synergy Advance provides you with instant feedback during editing to let you know whether the current format string is valid. It does this by showing a small triangular warning icon next to any invalid format string in the user interface. Clicking on the warning icon brings you to this help page in your web browser.</p>
          
          <h4>Literal percent symbols</h4>
          
          <p>So if the percent symbol is used to introduce a format marker, how do you include a literal percent symbol in your format string? The answer is to immediately follow the percent symbol by a second percent symbol; Synergy Advance will interpret that to mean "literal percent symbol" and will print a single percent symbol in the output. Thus, this format string:</p>
          
          <pre>100%% Gold!</pre>
          
          <p>Will be printed as follows in the final output:</p>
          
          <pre>100% Gold!</pre>
          
          <h4>Where are format strings used in Synergy Advance?</h4>
          
          <p>Synergy Advance uses format strings in places where it must output textual (non-graphical) information. For example, if you use Synergy Advance to send Growl notifications or you use it to update your iChat status line automatically. In both cases the target which receives that information (Growl or iChat respectively) expects to receive a textual string. If you look at the <a href="../preferences/instant-messaging/">Instant Messaging</a> or <a href="../preferences/growl/">Growl</a> preference panes you will see spaces in which you can enter custom format strings. The user interface displays a list of available format markers so that you don't have to constantly refer to the documentation to remind yourself what the different markers are.</p>
          
          <h4>Format string reference</h4>
          
          <table>
            <tr>
              <td>%t</td><td>track title</td>
            </tr>
            <tr>
              <td>%a</td><td>artist</td>
            </tr>
            <tr>
              <td>%c</td><td>composer</td>
            </tr>
            <tr>
              <td>%b</td><td>album</td>
            </tr>
            ...
            <tr>
              <td>%p</td><td>player state ("Playing", "Paused", "Stopped")</td>
            </tr>
            <tr>
              <td>%z</td><td>year</td>
            </tr>
            <tr>
              <td>%r</td><td>rating (displayed as stars)</td>
            </tr>
            <tr>
              <td>%n</td><td>track number</td>
            </tr>
            <tr>
              <td>%k</td><td>track count</td>
            </tr>
            <tr>
              <td>%d</td><td>disc number</td>
            </tr>
            <tr>
              <td>%u</td><td>disc count</td>
            </tr>
            <tr>
              <td>%g</td><td>grouping</td>
            </tr>
            <tr>
              <td>%i</td><td>total time</td>
            </tr>
            <tr>
              <td>%x</td><td>date added</td>
            </tr>
            <tr>
              <td>%m</td><td>BPM</td>
            </tr>
            <tr>
              <td>%y</td><td>play count</td>
            </tr>
            <tr>
              <td>%%</td><td>literal %</td>
            </tr>
