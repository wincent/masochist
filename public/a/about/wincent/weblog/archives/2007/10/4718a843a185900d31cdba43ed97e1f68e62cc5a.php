...
<title>Wincent Colaiuta's weblog: Creating new daemon users on Leopard</title>
...
<link rel="prev" href="https://wincent.com/a/about/wincent/weblog/archives/2007/10/on_leopards_sta.php" title="On Leopard's stability" />
...
<a href="https://wincent.com/a/about/wincent/weblog/archives/2007/10/on_leopards_sta.php">&laquo; On Leopard's stability</a> |
...
<h3>Creating new daemon users on Leopard</h3>

<p>I just made a curious realization while trying to build <a href='https://wincent.com/knowledge-base/MySQL'>MySQL</a> for <a href='https://wincent.com/knowledge-base/Leopard'>Leopard</a>: I don't know how to create new daemon user. No doubt this is ridiculously easy with Mac OS X Server, but in the non-server version it's far from obvious.</p>
<p>The NetInfo Manager utility has finally be laid to rest, but so have <a href='https://wincent.com/knowledge-base/command%20line'>command line</a> tools like <tt>adduser</tt>. The <tt>/etc/passwd</tt> file starts with this info:</p>
<pre>##
# User Database
# 
...
# Open Directory.
##</pre>
<p>So what am I supposed to do? Use the Directory Utility application to turn on the BSD local node and then manually edit <tt>/etc/passwd</tt> and set a password using the <tt>passwd</tt> tool (for which there is no longer a manpage, but at least <tt>password --foo</tt> causes usage information to be printed)? Is there a more elegant way than this? What does Mac OS X Server do under the hood?</p>
<a name='Update'></a><h3>Update</h3>

<ul>
<li>Luckily, thanks to somebody who didn't know that Leopard was under an <a href='https://wincent.com/knowledge-base/NDA'>NDA</a>, found <a href='http://www.michaelnygard.com/blog/2007/04/moving_your_home_directory_on.html'>this article</a> published back in April describing the <tt>dscl</tt> tool, a command line interface for Directory Services.</li>
<li><a href='http://docs.info.apple.com/article.html?artnum=306494'>http://docs.info.apple.com/article.html?artnum=306494</a></li>
<li><a href='http://www.royhooper.ca/blog/articles/2007/10/27/creating-os-only-users-on-leopard'>http://www.royhooper.ca/blog/articles/2007/10/27/creating-os-only-users-on-leopard</a></li>
</ul>
<p>So it looks like something like this <em>would</em> probably work:</p>
<pre>sudo dscl localhost -create /Local/Default/Users/mysql
...
sudo dscl localhost -create /Local/Default/Users/_mysql UserShell: /usr/bin/false</pre>
<p><em>But</em>, having started to snoop around using <tt>dscl</tt> I saw that there already is a MySQL user, it's just that it appears as <tt>_mysql</tt>. Information about it can be printed by issuing one of the following commands:</p>
<pre># with leading underscore
dscl localhost -cat /Local/Default/Users/_mysql

# without
dscl localhost -cat /Local/Default/Users/mysql</pre>
<p>The output is:</p>
<pre>AppleMetaNodeLocation: /Local/Default
...
<div id="a002858more"><div id="more">

</div></div>

<p class="more-categories"></p>

<p class="posted">Posted by wincent at October 30, 2007  3:07 PM</p>
