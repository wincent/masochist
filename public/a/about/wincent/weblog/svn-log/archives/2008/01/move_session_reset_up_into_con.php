...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>
      Git activity log: Move session reset up into controller (wincent.dev,
      ab81739)
    </title>
    ...
    <link
      rel="prev"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/01/updates_for_session_fixation_a.php"
      title="Updates for session fixation attacks (wincent.dev, ba7907d)"
    />

    <link
      rel="next"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/01/add_in_place_editing_plugin_wi.php"
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/01/updates_for_session_fixation_a.php"
            >&laquo; Updates for session fixation attacks (wincent.dev,
            ba7907d)</a
          >
          |
          ...
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2008/01/add_in_place_editing_plugin_wi.php"
            >Add in_place_editing_plugin (wincent.dev, 209d96c) &raquo;</a
          >
        </p>

        <h2>January 25, 2008</h2>

        <h3>Move session reset up into controller (wincent.dev, ab81739)</h3>

        <p>
          We must reset the session from inside the controller and not in
          authentication.rb (the current_user= method).
        </p>

        <p>
          If we try the latter then we get
          ActionController::InvalidAuthenticityToken exceptions during login
          because the code is run in a before filter (specifically, we go to our
          sessions &quot;new&quot; action, this calls the session
          &quot;create&quot; action, the before filter runs and
          login_with_cookies is called and fails because we are not logged in
          yet, so the current_user is set to nil, which in turn resets the
          session and the filters continue running, eventually getting to the
          &quot;protect_from_forgery&quot; one which fails and throws an
          exception because just cleared our session).
        </p>

        <p>
          We must only clear the session after the before filters run, and the
          correct place to do that is in the sessions controller
