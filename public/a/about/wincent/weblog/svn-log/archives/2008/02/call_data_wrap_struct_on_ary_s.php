...
<title>Git activity log: Call Data_Wrap_Struct on ary structs (wikitext, 7f506d3)</title>
...
<link rel="prev" href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/extract_ary_free_out_into_sepa.php" title="Extract ary_free() out into separate file (wikitext, 840b62d)" />


<link rel="next" href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/add_gc_wrap_ary_macro_wikitext.php" title="Add GC_WRAP_ARY macro (wikitext, f4b4579)" />
...
<a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/extract_ary_free_out_into_sepa.php">&laquo; Extract ary_free() out into separate file (wikitext, 840b62d)</a> |

<a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/">Main</a>

| <a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/add_gc_wrap_ary_macro_wikitext.php">Add GC_WRAP_ARY macro (wikitext, f4b4579) &raquo;</a>

</p>

<h2>February 23, 2008</h2>

<h3>Call Data_Wrap_Struct on ary structs (wikitext, 7f506d3)</h3>

<p>There is a minuscule overhead introduced by this commit (the allocation of three Ruby objects per parse, which in turn means three more objects involved in Ruby's mark-and-sweep Garbage Collection scheme) which brings with it a robustness enhancement: if we exit the parse method irregularly (by an exception) the three ary structs will be properly freed rather than leaking.</p>

<p>This was never a big deal as the structs themselves were typically small (defaulting to 64 entries each) and irregular exits shouldn't really ever happen in practice (the only times when we would exit via an exception are when we run out of memory, in which case we're totally hosed anyway and process termination is just around the corner, and when we have invalidly encoded UTF-8 input; this latter scenario may be an issue if an attacker specifically tries to consume resources by repeatedly feeding in hand-crafted bad input, but I don't know whether such an attack would be practically feasible).</p>

<p>The key piece of the puzzle is that the object returned by Data_Wrap_Struct must be locatable on the C stack in order for Ruby to refrain from collecting it prematurely. Without the volatile keyword the compiler will see that the pointer is initialized and then never used again and is free to optimize it away to nothing, which then allows Ruby to go ahead and free the structure before its time. The volatile keyword here prevents this optimization, at least in GCC, and is apparently an &quot;accepted hack&quot; that is used in the Ruby source itself.</p>
...
<p class="posted">Posted by wincent at February 23, 2008  3:55 PM</p>
...
<div id="footer">
  ...
  <p>Page last updated 00:30:18, 19 February 2010.</p>
  <p>
