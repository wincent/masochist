...
<title>Git activity log: Add invalid_encoding test to benchmarks subdirectory (wikitext, 29af551)</title>
...
<link rel="prev" href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/wrap_str_structs_with_data_wra.php" title="Wrap str structs with Data_Wrap_Struct (wikitext, 4389475)" />


<link rel="next" href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/add_one_longrunning_spec_to_gu.php" title="Add one long-running spec to guard against GC and memory-related bugs (wikitext, c0cc9c3)" />
...
<body>
  ...
  <div id="breadcrumbs">
    <p><a href="http://www.wincent.com/">Main</a>&nbsp;&gt;&nbsp;<a href="../../../../../../">About</a>&nbsp;&gt;&nbsp;<a href="../../../../../">Wincent</a>&nbsp;&gt;&nbsp;<a href="../../../../">Weblog</a>&nbsp;&gt;&nbsp;<a href="../../../">Git log</a>&nbsp;&gt;&nbsp;<a href="../../">Archives</a>&nbsp;&gt;&nbsp;<a href="../">2008</a>&nbsp;&gt;&nbsp;02    </p>
  </div>
...
<a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/wrap_str_structs_with_data_wra.php">&laquo; Wrap str structs with Data_Wrap_Struct (wikitext, 4389475)</a> |

<a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/">Main</a>

| <a href="http://www.wincent.com/a/about/wincent/weblog/svn-log/archives/2008/02/add_one_longrunning_spec_to_gu.php">Add one long-running spec to guard against GC and memory-related bugs (wikitext, c0cc9c3) &raquo;</a>

</p>

<h2>February 23, 2008</h2>

<h3>Add invalid_encoding test to benchmarks subdirectory (wikitext, 29af551)</h3>

<p>This isn't so much a benchmark as a test, but I don't want to include it in the specs directory because it is such a long-running script.</p>

<p>The basic idea is to parse a large slab of text 100,000 times and make sure that there is no resource leakage even when exiting abnormally from the parse method.</p>

<p>You can't really test this within a spec because the spec suite doesn't run for long enough for resource leaks to really manifest themselves (on my humble machine the spec suite of 564 examples runs in under a third of a second). I could bloat the spec suite to make it run for longer, but that would slow down development and detecting resource leaks from within the specs wouldn't be all that straightforward anyway.</p>

<p>So the idea is that you run this invalid_encoding script and watch memory usage while it processes the same slab 100,000 times over. The slab is long enough that the parser is fully exercised (things like indentation, various syntax etc), and only at the end do we introduce some invalidly encoded UTF-8 which causes the parse to derail.</p>

<p>It's important to do these long-running tests because when I was initially figuring out how to use Data_Wrap_Struct (before I knew about the need to use the volatile keyword) I was seeing the full spec suite passing, but crashes when running the long-running benchmarks. This was because Ruby's GC was never being triggered during the spec run and so the problem never manifested itself.</p>

<p>On my machine this script reports 2.55MB RSIZE, 18.53MB VSIZE, 1.66MB private and 820KB shared, and those figures don't budge at all during the entire run.</p>
...
<p class="posted">Posted by wincent at February 23, 2008  4:57 PM</p>
