...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <link
      rel="prev"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/09/record_filenames_gdiff_52e4e48.php"
      title="Record filenames (gdiff, 52e4e48)"
    />

    <link
      rel="next"
      href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/09/be_encodingagnostic_with_respe.php"
    ...
    <div id="container">
      <div class="content">
        <p align="right">
          <a
            href="https://wincent.dev/a/about/wincent/weblog/svn-log/archives/2007/09/record_filenames_gdiff_52e4e48.php"
            >&laquo; Record filenames (gdiff, 52e4e48)</a
          >
          |
        ...
        <p>
          Update the state machine to capture blob ids and store them in the
          WOFile objects. This is a simple ASCII capture which requires a
          pointer to be set on seeing the start of the desired substring and
          then actually creating a new object based on the subtring upon
          reaching the end.
        </p>

        <p>
          Note that these are abbreviated SHA-1 hashes, not full numeric hashes,
          so I store them as strings rather than numbers. I will later be
          passing these strings to other Git tools (such as git-show) so there
          is no point in using a numeric representation.
        </p>

        <p>
          Given that this requires the addition of the ASCII capture machinery I
          also use it for capturing unquoted paths, seeing as it is simpler and
          faster.
        </p>

        <p>
          At this point I did some experimentation and discovered that quoted
          paths and numeric escapes of the form &quot;\ddd&quot; are used
          whenever a path contains non-ASCII characters in it. This commit
          includes some proof-of-concept code for recognizing and recording
          these numeric escapes, although as noted in the comments to make it
          actually work I'll need to instead capture raw bytes one by one (most
          likely into an NSMutableData object) and finally converting the data
          to a string in one shot rather than working at the per-character level
          as I currently do.
        </p>

        <p>Signed-off-by: Greg Hurrell &lt;greg@hurrell.net&gt;</p>

        <div id="a002928more"><div id="more"></div></div>
